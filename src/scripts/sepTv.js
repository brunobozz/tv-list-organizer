...
fs.readFile("src/files/listas/br01.m3u", "utf8", (err, data) => {
  if (err) {
  ...
  items.forEach((item) => {
    ...
    if (groupTitleMatch) {
      if (groupTitleMatch[1].includes("|")) {
        const groupTitleSplit = groupTitleMatch[1].split(" | ");
        linha.type = groupTitleSplit[0];
        linha.category = groupTitleSplit[1];
      }
      const tvgNameMatch = extInfLine.match(/tvg-name="([^"]+)"/);
      if (tvgNameMatch) {
        linha.title = tvgNameMatch[1];
        console.log("----------------------------");
        console.log(linha);
